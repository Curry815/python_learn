...
result1 = file_rdd.map(lambda x: (x.split("\t")[0][:2], 1) ).\
    ...
    take(3)
print(f"需求1的结果：{result1}") # [('20', 3479), ('23', 3087), ('21', 2989)]
...
result2 = file_rdd.map(lambda x: (x.split("\t")[2], 1)).\
    ...
    sortBy(lambda x: x[1], False, numPartitions=1).\
    take(3)
print(f"需求2的结果：{result2}") # [('scala', 2310), ('hadoop', 2268), ('博学谷', 2002)]

# TODO 需求3：统计黑马程序员关键词在什么时段被搜索的最多
...
result3 = file_rdd.map(lambda x: x.split("\t")).\
    ...
    sortBy(lambda x: x[1], False, numPartitions=1).\
    take(1)
print(f"需求3的结果：{result3}") # [('22', 245)]

# TODO 需求4：将数据转换成JSON格式，写出到文件中
# 4.1 转换为JSON格式的RDD
# 4.2 写出为文件
file_rdd.map(lambda x: x.split("\t")).\
    map(lambda x: {"time": x[0], "user_id": x[1], "key_word": x[2], "rank1": x[3], "rank2": x[4], "url": x[5]}).\
    saveAsTextFile("F:/search_log_json.txt")
